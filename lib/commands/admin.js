...
const { buildRuleText } = require('../command-data');
...
async function handleAdmin(cmd, args, userId, username, ctx) {
  ...
  if (cmd === '/mutelist') {
    ...
    for (const [uid, info] of [...session.mutedUsers]) {
      if (info.until > now) {
        list.push(`${uid}（by: ${info.mutedById || '__system__'} / 残り ${Math.ceil((info.until - now) / 1000)}秒）`);
      } else {
  ...
  if (cmd === '/shadowbanlist') {
    const list = [...session.shadowBannedUsers].map(uid => `${uid}（by: ${session.shadowBanById.get(uid) || '__system__'}）`);
    return list.length === 0
