...
const { asCallback, onDbErr, sanitizeReplyTo, buildCurrentMessage, buildCommandResultMessages, saveMessage, normalizeMessageColor, } = require('./shared');

function emitPrivateMessageDelivery(io, session, socket, pmData, { shadowBanned = false } = {}) {
    socket.emit('privateMessage', pmData);
    if (shadowBanned) {
        return;
    }
    emitToUserSockets(io, session, pmData.toId, 'privateMessage', pmData);
    for (const sid of session.adminSockets) {
        const adminUserId = session.getUserIdBySocket(sid);
        if (adminUserId !== pmData.fromId && adminUserId !== pmData.toId) {
            io.sockets.sockets.get(sid)?.emit('privateMessageMonitor', pmData);
        }
    }
}
async function emitShadowBannedCommandResponse(io, session, socket, currentMessage, result) {
    if (result === null) {
    ...
    if (result.type === 'private') {
        if (result.pmData) {
            emitPrivateMessageDelivery(io, session, socket, result.pmData, { shadowBanned: true });
        }
    ...
    socket.on('sendMessage', async (data, cb) => {
        ...
        if (isShadowBanned) {
            if (message.startsWith('/')) {
                ...
                const payload = await emitShadowBannedCommandResponse(io, session, socket, currentMessage, result);
                return cb(payload);
            }
        ...
        if (message.startsWith('/')) {
            ...
            if (result.type === 'private') {
                if (result.pmData) {
                    emitPrivateMessageDelivery(io, session, socket, result.pmData, { shadowBanned: false });
                }
                socket.emit('systemMessage', result.message);
                return cb({ success: true });
            }
